...
flags.DEFINE_integer('unroll_length', 500, 'Unroll length in agent steps.')
...
def calculate_gae(env_outputs, values, bootstrap_value):
  ...
  GAE_discounts = tf.to_float(~done) * FLAGS.discounting * FLAGS.GAE_discounting
  sequences = (
    tf.reverse(GAE_discounts, axis=[0]),
    tf.reverse(deltas, axis=[0]),
  )
    # GAE
  def scanfunc(acc, sequence_item):
    discount_t, delta_t = sequence_item
    return delta_t + discount_t * acc
  initial_values = tf.zeros_like(bootstrap_value)
  pg_advantages = tf.scan(
      fn=scanfunc,
      elems=sequences,
  ...
  pg_advantages = tf.reverse(pg_advantages, [0], name='pg_advantages')
  return tf.stop_gradient(pg_advantages)
...
def compute_ppo_loss(logits, actions, advantages):
  log_prob = -tf.nn.sparse_softmax_cross_entropy_with_logits(labels=actions,
          logits=logits)
  log_prob_old = tf.stop_gradient(log_prob)
  ...
  with tf.Graph().as_default(), \
    ...
    with tf.train.MonitoredTrainingSession(
        ...
        hooks=[py_process.PyProcessHook(),
               #tf_debug.TensorBoardDebugHook("localhost:7000")
               ]) as session:
